 ...
 * `crypto.randomUUID` exists only in secure contexts (HTTPS or loopback), so a
 * plain-HTTP LAN deployment (the all-interfaces bind) cannot rely on it; every
 * browser exposes `getRandomValues` on insecure origins.
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    // getRandomValues-backed UUID: platform-neutral and present on insecure origins.
